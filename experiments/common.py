...
import torch
import tqdm
BASELINE_MODELS = ['ncde', 'odernn', 'dt', 'decay', 'gruode','odernn_forecasting','ncde_forecasting','decay_forecasting','dt_forecasting','gruode_forecasting','double_ncde_new6']
BASELINE_MODELS_F = ['ncde', 'odernn', 'dt', 'decay', 'gruode','odernn_forecasting','decay_forecasting','dt_forecasting']
...
def _evaluate_metrics(model_name,dataloader, model, times, loss_fn, num_classes,slope, device,kwargs):
    ...
    with torch.no_grad():
        ...
        if model_name in LEARNABLEPATH:
            ...
            metrics = _AttrDict(accuracy=total_accuracy.item(), confusion=total_confusion, dataset_size=total_dataset_size,
                            loss=total_loss.item(),mse_loss=mse_loss.item(),h_loss = h_loss.item())
...
def _evaluate_metrics_forecasting(model_name, dataloader, model, times, loss_fn, num_classes,slope, device,kwargs):
...
def _train_loop_forecasting(model_name, train_dataloader, val_dataloader, test_dataloader,model, times, optimizer, loss_fn, max_epochs, num_classes,slope_check, device,
                kwargs, step_mode):
    ...
    for epoch in tqdm_range:
        ...
        if epoch % epoch_per_metric == 0 or epoch == max_epochs - 1:
            model.eval()
            train_metrics = _evaluate_metrics_forecasting(model_name, train_dataloader, model, times, loss_fn, num_classes,slope, device, kwargs)
            val_metrics = _evaluate_metrics_forecasting(model_name, val_dataloader, model, times, loss_fn, num_classes,slope, device, kwargs)
            test_metrics = _evaluate_metrics_forecasting(model_name, test_dataloader, model, times, loss_fn, num_classes,slope, device,kwargs)
            ...
            tqdm_range.write('Epoch: {}  Train MSE loss: {:.3}  '
                            'Val MSE loss: {:.3} Test MSE loss {:.3} slope: {:.5}  '
                            ''.format(epoch, train_metrics.loss,
                             val_metrics.loss,test_metrics.loss, slope))
...
def _train_loop(model_name,experiments,train_dataloader, val_dataloader,test_dataloader, model, times, optimizer, loss_fn, max_epochs, num_classes,slope_check, device,
                kwargs, step_mode,c1,c2):
    ...
    if step_mode == 'trainloss':
        ...
        epoch_per_metric = 1
        plateau_terminate = 50
        scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(optimizer, patience=10,mode='max')
    ...
    for epoch in tqdm_range:
        ...
        for batch in train_dataloader:
            ...
            with _SuppressAssertions(tqdm_range):
                *train_coeffs, train_y, lengths = batch
                ...
                if model_name in LEARNABLEPATH:
                    pred_y,loss_1,loss_2 = model(times, train_coeffs, lengths,slope, **kwargs)
        ...
        if epoch % epoch_per_metric == 0 or epoch == max_epochs - 1:
            model.eval()
            train_metrics = _evaluate_metrics(model_name,train_dataloader, model, times, loss_fn, num_classes,slope, device, kwargs)
            val_metrics = _evaluate_metrics(model_name,val_dataloader, model, times, loss_fn, num_classes,slope, device, kwargs)
            test_metrics = _evaluate_metrics(model_name,test_dataloader, model, times, loss_fn, num_classes,slope, device, kwargs)
            ...
            if num_classes == 2 : 
                ...
                tqdm_range.write('Epoch: {}  Train loss: {:.3}  Train AUC : {:.3}  '
                                                'Val loss: {:.3} Val AUC : {:.3} Test loss : {:.3} Test AUC :{:.3} slope: {:.5}  '
                                                ''.format(epoch, train_metrics.loss, train_metrics.auroc,
                                                 val_metrics.loss, val_metrics.auroc,test_metrics.loss, test_metrics.auroc,slope))
                ...
                tqdm_range.write('Epoch: {}  Train loss: {:.3}  Train accuracy: {:.3}  '
                                                ...
                                                ''.format(epoch, train_metrics.loss, 
                                                train_metrics.accuracy, val_metrics.loss,val_metrics.accuracy,test_metrics.loss,test_metrics.accuracy))
...
def main(experiments,model_name,name, times, train_dataloader, val_dataloader, test_dataloader, device, make_model, num_classes, max_epochs,
         lr,slope_check, kwargs, step_mode,c1,c2, pos_weight=torch.tensor(1)):
    ...
    if slope_check :
        ...
        slope = 0.0
    train_metrics = _evaluate_metrics(train_dataloader, model, times, loss_fn, num_classes,slope, device, kwargs)
    val_metrics = _evaluate_metrics(val_dataloader, model, times, loss_fn, num_classes,slope, device, kwargs)
    test_metrics = _evaluate_metrics(test_dataloader, model, times, loss_fn, num_classes,slope, device,kwargs)
    if num_classes ==2:
...
def main_forecasting(experiments,model_name, name, times, train_dataloader, val_dataloader, test_dataloader, device, make_model, num_classes, max_epochs,
         lr,slope_check, kwargs, step_mode, pos_weight=torch.tensor(1)):
    ...
    optimizer = torch.optim.Adam(model.parameters(), lr=lr, weight_decay = 1e-4)
    # optimizer = torch.optim.Adam(model.parameters(), lr=lr)
    history, epoch = _train_loop_forecasting(model_name, train_dataloader, val_dataloader,test_dataloader, model, times, optimizer, loss_fn, max_epochs,
                          num_classes,slope_check, device, kwargs, step_mode)
...
def make_model(name, input_channels, output_channels, hidden_channels, hidden_hidden_channels,attention_channel,attention_attention_channel, num_hidden_layers,
               use_intensity,slope_check,soft,timewise,rtol,atol,file, initial):
    ...
    if name == 'ncde':
        ...
        def make_model():
            vector_field_f = models.FinalTanh_ff(input_channels=input_channels, hidden_atten_channels=attention_channel,
                                            hidden_hidden_atten_channels=attention_attention_channel,
                                            num_hidden_layers=num_hidden_layers)
            vector_field_g = models.FinalTanh(input_channels=input_channels, hidden_channels=hidden_channels,
                                            hidden_hidden_channels=hidden_hidden_channels,
                                            num_hidden_layers=num_hidden_layers)
            model = models.DoubleNeuralCDE(func=vector_field_f,func_g = vector_field_g, input_channels=input_channels, hidden_channels=hidden_channels,
                                     output_channels=output_channels,attention_channel=attention_channel,slope_check = slope_check,soft = soft,timewise=timewise, initial=initial)
        ...
        def make_model():
            ...
            vector_field_g = models.FinalTanh(input_channels=input_channels, hidden_channels=hidden_channels,
                                            hidden_hidden_channels=hidden_hidden_channels,
                                            num_hidden_layers=num_hidden_layers)
            model = models.DoubleNeuralCDE_debug(func=vector_field_f,func_g = vector_field_g, input_channels=input_channels, hidden_channels=hidden_channels,
                                     output_channels=output_channels,attention_channel=attention_channel,slope_check = slope_check,soft = soft,timewise=timewise, initial=initial)
            return model, vector_field_g,vector_field_f
    elif name == 'double_ncde_socar':
        def make_model():
            vector_field_f = models.FinalTanh_socar(input_channels=input_channels, hidden_atten_channels=attention_channel,
                                            hidden_hidden_atten_channels=attention_attention_channel,
                                            num_hidden_layers=num_hidden_layers)
            vector_field_g = models.FinalTanh(input_channels=input_channels, hidden_channels=hidden_channels,
                                            hidden_hidden_channels=hidden_hidden_channels,
                                            num_hidden_layers=num_hidden_layers)
            model = models.DoubleNeuralCDE_socar(func=vector_field_f,func_g = vector_field_g, input_channels=input_channels, hidden_channels=hidden_channels,
                                     output_channels=output_channels,attention_channel=attention_channel,slope_check = slope_check,soft = soft,timewise=timewise, initial=initial)
            return model, vector_field_g,vector_field_f
    
    elif name == 'ancde':
        def make_model():
            vector_field_f = models.FinalTanh_ff6( input_channels=input_channels, hidden_atten_channels=attention_channel,
                                            hidden_hidden_atten_channels=attention_attention_channel,
                                            num_hidden_layers=num_hidden_layers)
            vector_field_g = models.FinalTanh(input_channels=input_channels, hidden_channels=hidden_channels,
                                            hidden_hidden_channels=hidden_hidden_channels,
                                            num_hidden_layers=num_hidden_layers)
            model = models.ANCDE(func=vector_field_f,func_g = vector_field_g, input_channels=input_channels, hidden_channels=hidden_channels,
                                     output_channels=output_channels,attention_channel=attention_channel,slope_check = slope_check,soft = soft,timewise=timewise,file=file, initial=initial)
            return model, vector_field_g,vector_field_f

    elif name == 'Learnable_Path':
        def make_model():
            
            vector_field_f = models.FinalTanh_f(input_channels=input_channels,hidden_channels=hidden_channels,
                                            hidden_hidden_channels=hidden_hidden_channels,
                                            num_hidden_layers=num_hidden_layers)
            vector_field_g = models.FinalTanh_g(input_channels=input_channels, hidden_channels=hidden_channels,
                                            hidden_hidden_channels=hidden_hidden_channels,
                                            num_hidden_layers=num_hidden_layers)
            vector_field_hide = models.FinalTanh_hide(input_channels=input_channels,hidden_channels=hidden_channels,
                                            hidden_hidden_channels=hidden_hidden_channels,
                                            num_hidden_layers=num_hidden_layers)
            model = models.Learnable_Path(func=vector_field_f,func_g = vector_field_g,func_h=vector_field_hide, input_channels=input_channels, hidden_channels=hidden_channels,
                                     output_channels=output_channels, initial=initial,rtol=rtol,atol=atol)
            return model, vector_field_g,vector_field_f
        ...
        def make_model():
            ...
            vector_field_g = models.FinalTanh(input_channels=input_channels, hidden_channels=hidden_channels,
                                            hidden_hidden_channels=hidden_hidden_channels,
                                            num_hidden_layers=num_hidden_layers)
            model = models.ANCDE_forecasting(func=vector_field_f,func_g = vector_field_g, input_channels=input_channels,output_time=output_time, hidden_channels=hidden_channels,
                                     output_channels=output_channels,attention_channel=attention_channel,slope_check = slope_check,soft = soft,timewise=timewise,file=file, initial=initial)
         ...
         def make_model():
            vector_field = models.FinalTanh(input_channels=input_channels, hidden_channels=hidden_channels,
                                            hidden_hidden_channels=hidden_hidden_channels,
                                            num_hidden_layers=num_hidden_layers)
            model = models.NeuralCDE_forecasting(func=vector_field, input_channels=input_channels, hidden_channels=hidden_channels,
                                     output_channels=output_channels, initial=initial)
            return model, vector_field       
        ...
        def make_model():
            vector_field = models.GRU_ODE(input_channels=input_channels, hidden_channels=hidden_channels)
            model = models.NeuralCDE_forecasting(func=vector_field, input_channels=input_channels,
                                     hidden_channels=hidden_channels, output_channels=output_channels, initial=initial)
            return model, vector_field
    elif name == 'dt':
        def make_model():
        ...
        def make_model():
            ...
            model = models.ODERNN_forecasting(input_channels=input_channels, hidden_channels=hidden_channels,
                                  hidden_hidden_channels=hidden_hidden_channels, num_hidden_layers=num_hidden_layers,
                                  output_channels=output_channels, use_intensity=use_intensity)
            return model, model
    else:
